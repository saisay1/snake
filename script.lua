...
local function applyPlayerESP(p)
    ...
    local teamColor = (p.Team and p.Team.TeamColor and p.Team.TeamColor.Color) or Color3.new(1, 1, 1)
    ...
    if healthBack then
        ...
        if bar and State.ShowHealth and hum.MaxHealth > 0 then
...
local function removePlayerESP(p)
    local cache = playerESPCache[p]
    if cache then
        pcall(function() if cache.hl and cache.hl.Parent then cache.hl:Destroy() end end)
        pcall(function() if cache.bb and cache.bb.Parent then cache.bb:Destroy() end end)
...
local function removeObjectESP(part)
    ...
    if esp then
        pcall(function() if esp.Parent then esp:Destroy() end end)
...
local function getTargetData(name)
    ...
    for _, t in ipairs(CustomTargets) do
        if string.find(nl, t.Name:lower(), 1, true) then
            return t
        end
    end
    return nil
...
if ParentGui:FindFirstChild("SaiBedwarsEspUni") then
    ParentGui.SaiBedwarsEspUni:Destroy()
end
...
OpenBtn.Text = "SAI"
OpenBtn.TextColor3 = Color3.fromRGB(128, 0, 0)
...
SettingsPanel.Size = UDim2.new(0, 110, 0, 70)
SettingsPanel.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
SettingsPanel.Visible = false
SettingsPanel.ZIndex = 1000
...
Title.Text = "  sai bedwars esp uni"
...
FunctionScroll.Position = UDim2.new(0, 10, 0, 130)
FunctionScroll.Size = UDim2.new(1, -20, 0, 135)
FunctionScroll.BackgroundTransparency = 1
FunctionScroll.BorderSizePixel = 0
FunctionScroll.CanvasSize = UDim2.new(0, 0, 2.5, 0)
FunctionScroll.ScrollBarThickness = 3
...
local function addToggle(text, stateKey, modeKey)
    ...
    if modeKey then
        ...
        opt.Size = UDim2.new(0, 25, 0, 20)
        opt.Position = UDim2.new(1, -70, 0.5, -10)
        opt.Text = "..."
        opt.TextColor3 = Color3.new(0.6, 0.6, 0.6)
    ...
    bg.Position = UDim2.new(1, -38, 0.5, -9)
    ...
    local function update()
        if State[stateKey] then
            ...
            TweenService:Create(c, tweenInfo, {Position = UDim2.new(1, -16, 0.5, -7)}):Play()
        else
            bg.BackgroundColor3 = Color3.fromRGB(80, 80, 80)
            TweenService:Create(c, tweenInfo, {Position = UDim2.new(0, 2, 0.5, -7)}):Play()
        end
    end
...
addCircle(Color3.fromRGB(200, 0, 0),   "🛏", "Beds")
